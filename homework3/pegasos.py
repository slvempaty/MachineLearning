...
def objective_function(X, y, w, lamb):
    ...
    first_term=(lamb/2)*w_l2_norm
    wmulx=np.matmul(X,w)
    ymulwmulx=y*wmulx
    oneminymulwmulx = [1-x for x in ymulwmulx]
    oneminymulwmulx = np.array(oneminymulwmulx)
    dummy=np.zeros(oneminymulwmulx.shape)
    aftermax=np.maximum(oneminymulwmulx,dummy)
    summation=sum(aftermax.flatten().tolist())
    num=X.shape[0]
    secondterm=summation/num
    obj_value=first_term+secondterm
   # done   
...
def pegasos_train(Xtrain, ytrain, w, lamb, k, max_iterations):
    ...
    for iter in range(1, max_iterations + 1):
        ...
        for eachval in A_t:
         ...
         if (ytrain[eachval] * np.dot(w,Xtrain[eachval])) < 1 :
            
            temp_Sum+=np.multiply(Xtrain[eachval],ytrain[eachval])
          
                 
        eta= 1/(lamb*iter)
        
        omegahalf=((1-eta*lamb)*w) + ((eta/k) *  temp_Sum)
        
        temp= 1/ (np.sqrt(lamb) * np.linalg.norm(omegahalf))
        
        w= (np.minimum(1,temp))*omegahalf
        
        train_obj.append(objective_function(Xtrain, ytrain,w[:,np.newaxis], lamb))
...
def pegasos_test(Xtest, ytest, w, t = 0.):
    ...
    w=np.array(w)
    wmulx=np.matmul(Xtest,w)
    num=0
    for idx,i in enumerate(wmulx):
      if(i<t):
         temp=-1
      else:
         temp=1
