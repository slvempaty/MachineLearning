...
def logistic_train_ovr(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    Xtraincp=numpy.insert(Xtrain,0,1,axis=1)
    #print(Xtrain)
    #print(Xtraincp)               
    for j in range(max_iterations):
        summation=numpy.zeros(minvector.shape[0])
        for i in range(Xtrain.shape[0]):
            sig= sigmoid(numpy.dot(minvector,Xtraincp[i]))
            minyn=sig-ytrain[i]
            secondterm=minyn*Xtraincp[i]
            summation=numpy.add(summation,secondterm)
        mulstepsize=step_size*(summation/Xtrain.shape[0])
        newminvector=numpy.subtract(minvector,mulstepsize)
...
def logistic_test_ovr(Xtest, w_l, b_l):
    ...
    """
    # you need to fill in your solution here
    Xopt = numpy.zeros(Xtest.shape[0])
    for i in range(Xtest.shape[0]) :
         a=b_l+numpy.dot(w_l,Xtest[i])
         #print(numpy.dot(w,Xtestcp[i]))
         Xopt[i]= 1/(1+numpy.exp(-1*a))
...
def logistic_mul_train(Xtrain, ytrain, w, b, step_size, max_iterations):
    ...
    minvector=numpy.zeros((w.shape[0],w.shape[1]+1)) #C X D+1
    Xtraincp=numpy.insert(Xtrain,0,1,axis=1)
    

    for i in range(max_iterations):#no.of iterations for convergence
      gnk=numpy.zeros((minvector.shape[0],minvector.shape[1]))
    
      for j in range(ytrain.shape[0]):#for each training point
         temp=0
         for l in range(ytrain.shape[1]):
            temp=temp+numpy.exp(numpy.dot(minvector[l],Xtraincp[j]))
                     
         for k in range(ytrain.shape[1]):#for each class compute gnk
            gnk[k]=numpy.add(gnk[k], numpy.dot((numpy.exp(numpy.dot(minvector[k],Xtraincp[j]))/temp) - ytrain[j][k],Xtraincp[j]))
         
      for l in range(ytrain.shape[1]):
         minvector[l]=numpy.subtract(minvector[l],((step_size*gnk[l])/Xtraincp.shape[0]))
...
def logistic_mul_test(Xtest, w_l, b_l):
    ...
    test_pred = numpy.zeros((Xtest.shape[0],b_l.shape[0]))
    for i in range(test_pred.shape[0]):
      
      temp=0
      for l in range(test_pred.shape[1]):
         temp=temp+numpy.exp(b_l[l]+numpy.dot(w_l[l],Xtest[i]))
      for j in range(test_pred.shape[1]):
         test_pred[i][j] = (numpy.exp(b_l[j]+numpy.dot(w_l[j],Xtest[i])))/temp
