...
def main(main_params):
    ...
    for t in range(num_epoch):
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            # TODO (3)
            grad_abc=model['M2'].backward(h2,grad_m2)
            grad_xyz=model['nonlinear2'].backward(c2,grad_abc) 
            grad_m1=model['C2'].backward(m1,grad_xyz)                                                                  
        ...
        for i in range(int(np.floor(N_train / minibatch_size))):
            ...
            # previous and the next modules, respectively                                  #
            ################################################################################
            s1=model['C2'].forward(m1)                                                                 #
            s2=model['nonlinear2'].forward(s1)                                                          #
            m2=model['M2'].forward(s2)
        ...
        for i in range(int(np.floor(N_val / minibatch_size))):
            ...
            # TODO (5)
            ss1=model['C2'].forward(m1)                                                                  #
            ss2=model['nonlinear2'].forward(ss1)                                                          #
            m2=model['M2'].forward(ss2)  
