...
class linear_layer:
    ...
    def forward(self, X):
        ...
        #multiply each row of X with W - check if there are different w's for each node of layer
        m, n = X.shape
        dummy,output_D=self.params['W'].shape
        
        forward_output = np.zeros((m,output_D))
       # for i in range(m):
        #    forward_output[i] =self.params['b']+ np.dot(X[i],self.params['W'])
        forward_output=np.dot(X,self.params['W'])
        for i in range(m):
         forward_output[i] = forward_output[i] + self.params['b']  
        #print(X)
    ...
    def backward(self, X, grad):
        ...
        #print(grad.shape)
        m,n=X.shape
        grad1,grad2=grad.shape
        backward_output=np.zeros((m,n))
        #print(grad1,grad2)
        #self.gradient['W'] = np.zeros((n,grad2))
        #self.gradient['b'] = np.zeros((1,grad2))
...
class relu:
    ...
    def forward(self, X):
        ...
        #m, n = X.shape
        #dummy,output_D=self.params['W'].shape
        dummy = np.zeros(X.shape)
        forward_output=np.zeros(X.shape)
        forward_output=np.maximum(X,dummy)
...
class dropout:
    ...
    def backward(self, X, grad):
        ...
        backward_output=np.zeros(X.shape)
        #print(X.shape,self.mask.shape,grad.shape)
        backward_output=np.multiply(self.mask,grad)
        #print(backward_output[0])
        return backward_output
