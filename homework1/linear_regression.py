...
def linear_regression_noreg(X, y):
  ...
  #####################################################
  Xtrans=X.transpose()
  XXtransMul=np.dot(Xtrans,X)
  XXtransInv=np.linalg.inv(XXtransMul)
  secondMul=np.dot(XXtransInv,Xtrans)
  w=np.dot(secondMul,y)
  #####################################################		 
  return w
...
def regularized_linear_regression(X, y, lambd):
  ...
  sh=XXtransMul.shape
  #print(sh[1])
  lambdaI=np.identity(sh[1])*lambd
  Addreg=np.add(XXtransMul,lambdaI)
  XXtransInv=np.linalg.inv(Addreg)
  secondMul=np.dot(XXtransInv,Xtrans)
  w=np.dot(secondMul,y)
  #####################################################		 
  return w
...
def tune_lambda(Xtrain, ytrain, Xval, yval, lambds):
  ...
  error=[]
  for x in lambds[:]:
      w=regularized_linear_regression(Xtrain, ytrain, x)
      error.append(test_error(w, Xval, yval))
  bestlambda=lambds[error.index(min(error))] 
  #####################################################		 
  return bestlambda
...
def test_error(w, X, y):
  """
    ...
    """
  #1/2 w^T (X^TX)w − w^T (X^T y)
  
  #y^2 - 2*(X^Ty)Tw+w^TXTXw
  #wtrans=w.transpose()
  #Xtrans=X.transpose()
  #XtransxX=np.dot(Xtrans,X)
  #wtransxXtransxX=np.dot(wtrans,XtransxX)
  #Firstmul=(np.dot(wtransxXtransxX,w))
  #Firstmul=Firstmul/2
  #XtransxY=np.dot(Xtrans,y)
  #secondmul=np.dot(wtrans,XtransxY)
  #err=Firstmul-secondmul
  wtrans=w.transpose()
  ysquaredot=np.dot(y,y)
  Xtrans=X.transpose()
  secondterm=2*(np.dot((np.dot(Xtrans,y).transpose()),w))
  thirdterm=np.dot(np.dot(wtrans,Xtrans),np.dot(X,w))
  err=(ysquaredot-secondterm+thirdterm)/(X.shape[0])  
  return err
