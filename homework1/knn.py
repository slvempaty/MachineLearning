...
def compute_distances(Xtrain, X):
	...
	"""
	#####################################################
	shtrain=Xtrain.shape
	shtest=X.shape
	#shrange=shtrain[0]
	#print(shtrain,shtest,Xtrain[1])
	dists=np.empty((shtest[0],shtrain[0]))
	for i in range(shtrain[0]):
            a=Xtrain[i]
            for j in range(shtest[0]):
                b=X[j]
                dists[j,i]=np.linalg.norm(a-b)
	#####################################################		 
	return dists
...
def predict_labels(k, ytrain, dists):
	...
	"""
	#####################################################
	toplabels=np.empty(k,dtype=np.int32)
	ypred=np.empty(dists.shape[0])
	for i in range(dists.shape[0]):
            a=np.copy(dists[i])
            for j in range(k):
                minindex=np.argmin(a)
                toplabels[j] = ytrain[minindex]
                a[minindex]=np.amax(a)
            frequency = np.bincount(toplabels)
            ypred[i] = np.argmax(frequency)
	#####################################################
	return ypred
...
def find_best_k(K, ytrain, dists, yval):
	...
	for k in K:
            ...
            acc[cou] = compute_accuracy(yval, ypred)
            cou+=1
	best_k=K[np.argmax(acc)]
