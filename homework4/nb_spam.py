...
def nb_train_smoothing(X, Y, vocab_size, num_classes, alpha):
    ...
    for c in range(num_classes):
        count_c = Y.count(c)
        class_prob[c]=count_c / num_data

    count_spam= [alpha] * vocab_size
    count_ham= [alpha] * vocab_size

    #for v in range(vocab_size):
    for idx,doc in enumerate(X):
        if(Y[idx] == 1):
            for word in doc:
                count_spam[word[0]] += word[1]

        if(Y[idx] == 0):
            for word in doc:
                count_ham[word[0]] += word[1]

    #Add all numbers in a list

    vocab_spam=sum(count_spam)
    vocab_ham=sum(count_ham)

    for voc in range(vocab_size):
        class_word_prob[0,voc]=count_ham[voc]/vocab_ham
        class_word_prob[1,voc]=count_spam[voc]/vocab_spam
    ###################################################
...
def nb_train(X, Y, vocab_size, num_classes):
    ...
    count_spam= [0] * vocab_size
    count_ham= [0] * vocab_size
...
def nb_predict(X, class_prob, class_word_prob):
    ...
    for i,doc in enumerate(X):
        ...
        p_of_spam=np.log(class_prob[1])
        p_of_ham=np.log(class_prob[0])

        for words in doc:

            if (class_word_prob[0][words[0]]!=0 or class_word_prob[1][words[0]] !=0 ):

                if (class_word_prob[1][words[0]] ==0):

                    p_of_spam+= (words[1] * -1e50)

                else:

                    p_of_spam+= (words[1] * np.log(class_word_prob[1][words[0]]))



                if (class_word_prob[0][words[0]] ==0):

                    p_of_ham+= (words[1] * -1e50)

                else:

                    p_of_ham+=( words[1] * np.log(class_word_prob[0][words[0]]))
